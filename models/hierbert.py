...
class HierarchicalBert(nn.Module):
    ...
    def forward(self,
        ...
        tf_idfs_reshape = tf_idfs.contiguous().view(-1, tf_idfs.size(-1))
        ...
        # Shape of encoder_outputs = Tensor(4, 512, 768) = Array of 1.572.864
        encoder_outputs = self.encoder(input_ids=input_ids_reshape,
                                       tf_idfs=tf_idfs_reshape,
                                       attention_mask=attention_mask_reshape,
                                       token_type_ids=token_type_ids_reshape)[0]

        # Reshape back to (batch_size, n_segments, max_segment_length, output_size) --> (4, 4, 128, 768)
        # = Array of 1.572.864
